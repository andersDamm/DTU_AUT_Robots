...
typedef struct{ //input signals
		...
		double cr,cl;   // meters per encodertick
	        //output signals
		double right_pos,left_pos, ypos, xpos, theta;
		// internal variables
...
void update_odo(odotype *p)
{
  ...
  delta_u = ((double)delta_r + (double)delta_l)/2;
  delta_theta = ((double)delta_r - (double)delta_l)/(double)p->w;
  p->theta += delta_theta;
   ...
   switch (p->curcmd){
     ...
     case mot_move:
	  ...
	  p->motorspeed_r += AJAX*CONVERSION_FACTOR_ACC;
	        ...
	        p->motorspeed_l=p->speedcmd + K*(theta_ref-odo.theta);
          p->motorspeed_r=p->speedcmd - K*(theta_ref-odo.theta);
       }
     break;
